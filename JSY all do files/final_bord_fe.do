...
*** Log file
log using "C:\Users\ssj18\Documents\DHS\28oct2018_bord_fe.log", replace
...
* Second with cohorts of 10 - 15 years compared to 0-5 years
  ...
  reghdfe child_sex treat##post05 strend  if bord>=2 & b2>=2000 & b2<=2010,  cl(UID) absorb(UID b2 bord)
est store India_5_9
  reghdfe child_sex treat##post05 strend  if bord>=2 & b2>=2000 & b2<=2010 & rural==1,  cl(UID) absorb(UID b2 bord)
est store Rural_5_9
  reghdfe child_sex treat##p5 strend  if bord>=2,  cl(UID) absorb(UID b2 bord)
est store India_0_5
  reghdfe child_sex treat##p5 strend  if bord>=2 & rural==1,  cl(UID) absorb(UID b2 bord)
est store Rural_0_5
...
* Regression 3: Comparing JSY treatment (uppercaste rich in LPS) and control (uppercaste rich in HPS) groups within First Girl Families in all India and rural India
  ...
  reghdfe child_sex treat##post05 strend  if bord>=2 & b2>=2000 & b2<=2010 & fg==1,  cl(UID) absorb(UID b2 bord)
est store India_5_9
  reghdfe child_sex treat##post05 strend  if bord>=2 & b2>=2000 & b2<=2010 & rural==1 & fg==1,  cl(UID) absorb(UID b2 bord)
est store Rural_5_9
  reghdfe child_sex treat##p5 strend  if bord>=2 & fg==1,  cl(UID) absorb(UID b2 bord)
est store India_0_5
  reghdfe child_sex treat##p5 strend  if bord>=2 & rural==1 & fg==1,  cl(UID) absorb(UID b2 bord)
est store Rural_0_5
esttab India_5_9 Rural_5_9 India_0_5 Rural_0_5, replace b(%10.4f)se scalars(N  r2) star(* 0.10 ** 0.05 *** 0.01) mtitles long title(Impact on sex selective abortions)
//for Latex
*esttab India_5_9 Rural_5_9 India_0_5 Rural_0_5 , replace b(%10.4f)se scalars(N r2) star(* 0.10 ** 0.05 *** 0.01) mtitles long title(Impact on sex selective abortions) tex
...
* Regression 4: Robustness check for the program year effect.
  ...
  reghdfe child_sex treat##i.b2 strend  if bord>=2 & b2>=2000  & rural==1,  cl(UID) absorb(UID b2 bord )
est store Rural
  reghdfe child_sex treat##i.b2 strend  if bord>=2 &  b2>=2000 & rural==1 & fg==1,  cl(UID) absorb(UID b2 bord)
est store Rural_FG
